...
void ShiftLcd::write(const char msg[]){
  writeString(msg);
}

void ShiftLcd::write(const char msg1[], const char msg2[]){
  writeString(msg1);
  writeDisplay(LINE_TWO, WRITE_INSTR);
  writeString(msg2);
}

void ShiftLcd::write(const char msg[], int n){
...
void ShiftLcd::writePercent(const float percent) {
  const uint8_t numberOfTilesToLight = static_cast<uint8_t>(round(percent * 32.0f));
  uint8_t index = 0;
  while (index < numberOfTilesToLight && index < 16) {
    index++;
    writeDisplay(ALL_BLACK, WRITE_CHAR);
  }
  writeDisplay(LINE_TWO, WRITE_INSTR);
  while (index < numberOfTilesToLight && index < 32) {
    index++;
    writeDisplay(ALL_BLACK, WRITE_CHAR);
  }
...
//TODO: Change int to byte and try with hardware
